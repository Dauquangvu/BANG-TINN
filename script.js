...
// Proxy dự phòng cho Lời Bác Hồ dạy
...
async function loadWorldNews() {
  await loadRSSNews('world-news', RSS_THEGIOI, 'tin thế giới', 'https://vnexpress.net/the-gioi');
}
...
async function loadVietnamNews() {
  await loadRSSNews('vietnam-news', RSS_TRONGNUOC, 'tin trong nước', 'https://vnexpress.net/thoi-su');
}

// ── HÀM CHÍNH: LẤY TIN QUA RSS API ───────────────────────────
async function loadRSSNews(containerId, rssUrl, label, fallbackUrl) {
  ...
  try {
    ...
    // ── SỬA: tăng số bài lên 6, tăng độ dài description lên 600 ký tự ──
    const articles = data.items.slice(0, 6).map(item => {
      const pubDate = item.pubDate ? new Date(item.pubDate) : null;
      const dateStr  = pubDate ? formatNewsDate(pubDate) : 'Hôm nay';
      const timeAgo  = pubDate ? (Date.now() - pubDate.getTime()) : 0;
      return {
        title:       item.title  || '',
    ...
    container.innerHTML = `
      <div class="fallback-msg">
        ...
        <div>
          ...
          Xem trực tiếp tại:
          <a href="${urlFormats[0]}" target="_blank">${urlFormats[0]}</a>
        </div>
      </div>`;
    return;
  }
    ...
    container.innerHTML = `
      <div class="fallback-msg">
        ...
        <div>
          Chưa tải được lời Bác Hồ dạy hôm nay.
          Xem trực tiếp tại:
          <a href="${usedUrl}" target="_blank">${usedUrl}</a>
        </div>
      </div>`;
  }
}
